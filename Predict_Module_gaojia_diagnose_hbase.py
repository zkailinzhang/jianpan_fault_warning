...
class Predicthbase:
    def __init__(self,config,device_name,system_id,kks_all,kks,var_kks,start_row,end_row,interval):
        # start_1 = time.time()
            ...
            if self.kks and self.var_parameters:
                ...
                if lstm_model_dict:
                    ...
                    if data_predict_dict:
                        ...
                        # 计算预测次数
                        count = data_lenth-200
                        # 循环预测
                        for m in range(count):    
                            # lstm模型预测
                            for i in lstm_model_dict.keys():
                                data_predict = data_predict_dict[i]
                                data_predict = data_predict[m:m+200]
                                data_predict = savgol_filter(data_predict, 13, 3)
                            ...
                            for n in self.var_parameters:
                                for kks in n:
                                    data = data_predict_dict[kks]
                                    data =  data[m:m+200]
                                    var_predict_data[kks] = data
                            y_predicted_var = self.var.forecast(self.var_parameters, var_predict_data)
                            ...
                            for i in y_predicted_var.keys():
                                ...
                                #cf = self.conf.pre_hbase_insert_cf + ':' +str(self.system_id)+ ':' + i+ ':'+'evaluate'
                                value = float(y_predicted_var[i])
                                #value_1 = np.str(value)
                                
                                valuearray = data_value_dict[i]
                                real = valuearray[m+200]

                                value_1 = np.str('{},{}'.format(value,real))
                                time_1 = datetime.datetime.strptime( self.start_row, "%Y-%m-%d %H:%M:%S")
                                
                                time_row = (time_1 + datetime.timedelta(seconds=self.interval*m)).strftime("%Y-%m-%d %H:%M:%S")

                                #self.hbase.insert_data(Config.hbase_insert_table, time_row, cf, value_1)
                                self.hbase.insert_data(self.conf.pre_hbase_insert_table2, time_row, cf, value_1)
                            end_1 = time.time()
                        ...
                        if kks_name:
                            ...
                            if real_dist - mark_dist > 5:
                                ...
                                executor.submit(diagnose_task,warn_id)
