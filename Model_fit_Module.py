...
class ModelFIT:
    def __init__(self,config,system_id,kks,start_row,end_row):
        self.conf = config
        ...
        self.kks=kks
        self.interval=self.conf.interavl_lstm
        ...
        if self.kks:
            ...
            if data_dict:
                for i in data_dict.keys():
                    # data = savgol_filter(lstm_fit_dic[i], 13, 3)
                    data = data_dict[i]
                ...
                if data_fit_dict:
                    ...
                    if lstm_model_dict:
                        ...
                        print('模型训练失败!')
                        myLogger.write_logger('模型lsmt训练失败!')
                else:
                    print('未生成训练数据!')
            else:
                print('数据未提取成功!')
